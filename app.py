...
uploaded_file = st.sidebar.file_uploader(
    "Upload a CSV file with your financial data.", type=["csv"]
)
...
if any(feature not in financial_data.columns for feature in features):
    st.error(
        f"The uploaded file must contain the following columns: {', '.join(features)}"
    )
    st.stop()
...
for i in range(1, len(financial_data)):
    if financial_data.loc[i, "Anomaly"]:
        financial_data.loc[i, "Safe_Asset_Allocation"] = (
            financial_data.loc[i - 1, "Safe_Asset_Allocation"] * 1.005
        )
        financial_data.loc[i, "Risky_Asset_Allocation"] = (
            financial_data.loc[i - 1, "Risky_Asset_Allocation"] * 0.98
        )
    else:
        financial_data.loc[i, "Safe_Asset_Allocation"] = (
            financial_data.loc[i - 1, "Safe_Asset_Allocation"] * 0.997
        )
        financial_data.loc[i, "Risky_Asset_Allocation"] = (
            financial_data.loc[i - 1, "Risky_Asset_Allocation"] * 1.015
        )
...
best_period = financial_data.loc[financial_data["Total_Investment_Value"].idxmax()][
    "Date"
]
worst_period = financial_data.loc[financial_data["Total_Investment_Value"].idxmin()][
    "Date"
]
...
sns.lineplot(
    ...
    y=financial_data["Total_Investment_Value"],
    ax=ax,
...
st.subheader(
    "--- Dynamic Investment Strategy Report given an initial investment of 10,000 $ ---"
)
st.markdown(
    f"""
1. **Final Investment Value:** ${final_investment_value:,.2f}
